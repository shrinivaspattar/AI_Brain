#!/usr/bin/env python3
"""M32 real-T7 pilot expansion - a second, larger batch alongside the
original 2-file pilot (t7_chain2_pilot_batch.py), same methodology:
candidates hand-picked from the already-existing, already-hashed D1
exact_duplicate_groups report, never a fresh T7 scan.

NO REAL T7 PATH IS HARDCODED IN THIS FILE. The actual selection (five
real note groups, their known content hashes/sizes, and the derived
envelope) lives in the gitignored sibling
`t7_chain2_pilot_expansion_selection.json`.

This script does NOT open, read, or touch any T7 file itself - it only
creates database rows (one DiscoveryRun referencing the existing D1
report; one ClassificationRun/SourceInstance pair per selected
candidate path; one IngestionBatch) via the existing, unmodified
BatchCreationService. The five real files are only ever actually read
later, by the existing, unmodified scripts/run_ingestion_batch.py,
against the batch this script creates.

Run once, reviewed:
    python scripts/t7_chain2_pilot_expansion_batch.py
"""
...
SELECTION_PATH = Path(__file__).resolve().parent / "t7_chain2_pilot_expansion_selection.json"
D1_REPORT_PATH = Path(__file__).resolve().parents[1] / "knowledge" / "t7_discovery" / "duplicate_analysis.json"

_REQUIRED_KEYS = ("source_root", "groups", "envelope")
_REQUIRED_GROUP_KEYS = ("content_hash", "size_bytes", "paths")
...
def _load_selection() -> dict:
    if not SELECTION_PATH.is_file():
        raise SystemExit(
            ...
            "candidate groups for this expansion (see this script's module docstring)."
        )
    ...
    for i, group in enumerate(selection["groups"]):
        missing_g = [k for k in _REQUIRED_GROUP_KEYS if k not in group]
        if missing_g:
            raise SystemExit(f"{SELECTION_PATH} groups[{i}] is missing required keys: {missing_g}")
    return selection
    ...
    with Session(engine) as db:
        ...
        candidates = [
            CandidateObservation(
                root_t7_path=path,
                member_path=None,
                declared_size_bytes=group["size_bytes"],
                d1_duplicate_group_id=group["content_hash"],
            )
            for group in selection["groups"]
            for path in group["paths"]
        ]
        print(f"Built {len(candidates)} candidate observations across {len(selection['groups'])} content groups.")
        ...
        batch = BatchCreationService(db).create_batch(
            ...
            classifier_version="m32-real-t7-pilot-expansion-v1",
        )
