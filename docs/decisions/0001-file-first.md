...
to decide what the *source of truth* is: the filesystem, or the databases
(Postgres, ChromaDB) that index it.

## Decision
Files on disk are the source of truth. `documents/` (and later `knowledge/`)
hold the real data. Postgres rows (`Document`, `ImportJob`, ...) and Chroma
embeddings are derived indexes: metadata and vectors computed *from* the
...
## Consequences
- Rebuilding the index (Postgres + Chroma) from scratch is always possible
  and should be kept cheap, since it's the recovery path of last resort.
